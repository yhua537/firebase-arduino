...
void Transcriber::SetValue(const String& path, const String& value) {
...
void Transcriber::ProcessInitialUpdate(const FirebaseObject& update) {
  int digital_out = update.getInt((String("data") + kSubPathDigitalOut).c_str());
  if (!update.failed()) {
    digitalWrite(pin_digital_out_, digital_out);
  }

  float analog_out = update.getFloat((String("data") + kSubPathAnalogOut).c_str());
  if (!update.failed()) {
