...
FirebaseCloudMessaging::FirebaseCloudMessaging(const String& server_key) {
  auth_header_ = "key=";
...
const FirebaseError FirebaseCloudMessaging::SendMessageToUsers(
    const std::vector<String>& registration_ids,
    const FirebaseCloudMessage& message) {
  DynamicJsonBuffer buffer;
  JsonObject& root = buffer.createObject();
  JsonArray& ids = root.createNestedArray("registration_ids");
  for (const String& id : registration_ids) {
    ids.add(id.c_str());
  }
...
const FirebaseError FirebaseCloudMessaging::SendMessageToTopic(
    const String& topic, const FirebaseCloudMessage& message) {
  String to("/topics/");
  to += topic;
...
const void FirebaseCloudMessaging::AddToJson(
  ...
  if (message.collapse_key.length() != 0) {
  ...
  if (message.notification.title.length() != 0 || message.notification.body.length() != 0) {
    JsonObject& notification = json.createNestedObject("notification");
    if (message.notification.title.length() != 0) {
      notification["title"] = message.notification.title.c_str();
    }
    if (message.notification.body.length() != 0) {
