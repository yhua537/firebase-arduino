...
struct FirebaseCloudMessage {
  ...
  static FirebaseCloudMessage SimpleNotification(const String& title, const String& body);
};

// Firebase REST API client.
class FirebaseCloudMessaging {
 public:
  FirebaseCloudMessaging(const String& server_key);
  const FirebaseError SendMessageToUser(const String& registration_id,
                                        const FirebaseCloudMessage& message);
  const FirebaseError SendMessageToUsers(const std::vector<String>& registration_ids,
                                         const FirebaseCloudMessage& message);

  // TODO: The protocol supports sending a message to multiple topics but it is
  // not implemented here.
  const FirebaseError SendMessageToTopic(const String& topic,
                                         const FirebaseCloudMessage& message);
